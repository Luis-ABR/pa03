...
void Graph::updateNode(int id, NodeInfo node) {
    if (id < 0) {
        throw out_of_range("Node index can't be negative");
    }
    if (id >= size) {
        resize(id + 1);
        nodes[id] = new NodeInfo(node);
    } else {
        if (nodes[id] == nullptr) {
            nodes[id] = new NodeInfo(node);
        } else {
...
void Graph::updateConnection(int v, int u, double w) {
    if (v < 0 || v >= (int)nodes.size() || nodes[v] == nullptr) {
        cerr << "Attempting to update connection between "
             << v << " and " << u << " with weight " << w
             << " but " << v << " does not exist" << endl;
        exit(1);
    }
    if (u < 0 || u >= (int)nodes.size() || nodes[u] == nullptr) {
        cerr << "Attempting to update connection between "
             << v << " and " << u << " with weight " << w
             << " but " << u << " does not exist" << endl;
        exit(1);
    }
